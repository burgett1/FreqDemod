...
3. Explicitly overwrite the file with mode='w'. Run:
    Signal('{0}', mode='w', driver={1}, backing_store={2})""".format(
                        filename, driver, backing_store
                    )
            ...
            self.f = h5py.File(filename, driver="core", backing_store=False)
    ...
    def load_nparray(self, s, s_name, s_unit, dt, s_help="cantilever displacement"):
        ...
        attrs = OrderedDict(
            [
                ...
                (
                    "label_latex",
                    "${0} \: [\mathrm{{{1}}}]$".format(s_name, s_unit),
                ),
                ("help", s_help),
                ("abscissa", "x"),
                ("n_avg", 1),
    ...
    def load_hdf5(
        self, f, s_dataset="y", t_dataset="x", infer_dt=True, infer_attrs=True
    ):
    ...
    def fft(self):
        ...
        attrs = OrderedDict(
            [
                ...
                (
                    "label_latex",
                    "$\hat{{{0}}} \: [\mathrm{{{1}/Hz}}]$".format(name_orig, unit_orig),
                ),
    ...
    def fit_phase(self, dt_chunk_target):
        ...
        new_report.append(
            "A total of {0} chunks will be curve fit,".format(n_tot_chunk)
        )
        new_report.append(
            "corresponding to {0:.3f} ms of data.".format(1e3 * dt * n_total)
        )
    ...
    def fit_amplitude(self):
        ...
        * From the ``lmfit`` documentation [`link <http://newville.github.io/lmfit-py/fitting.html#fit_report>`__]:
        ...
        title = (
            "a(t) = a0*exp(-t/tau) + a1"
            "\n"
            "a0 = {0:.6f} +/- {1:.6f}, tau = {2:.6f} +/- {3:.6f}, a1 = {4:.6f} +/- {5:.6f}".format(
                p["a0"].value,
                p["a0"].stderr,
                p["tau"].value,
                p["tau"].stderr,
        ...
        title_LaTeX = (
            r"$a(t) = a_0 \exp(-t/\tau) + a_1$"
            "\n"
            r"$a_0 = {0:.6f} \pm {1:.6f}, \tau = {2:.6f} \pm {3:.6f}, a_1 = {4:.6f} \pm {5:.6f}$".format(
                p["a0"].value,
                p["a0"].stderr,
                p["tau"].value,
                p["tau"].stderr,
        ...
        attrs = OrderedDict(
            [
                ...
                (
                    "label_latex",
                    "$a_{{\mathrm{{calc}}}} \: [\mathrm{{{0}}}]$".format(a_unit),
                ),
        ...
        attrs = OrderedDict(
            [
                ...
                (
                    "label_latex",
                    "$a - a_{{\mathrm{{calc}}}} \: [\mathrm{{{0}}}]$".format(a_unit),
                ),
...
def testsignal_sine_exp():
    ...
    dt = 1 / fd
    t = dt * np.arange(nt)
    s = sn * np.sin(2 * np.pi * f0 * t) * np.exp(-t / tau) + np.random.normal(
        0, sn_rms, t.size
    )
